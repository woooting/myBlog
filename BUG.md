...
**注意**：`include` 必须显式包含类型依赖链：
- `nitro-nuxt.d.ts` → Nuxt 扩展
- `nitro.d.ts` → Nitro 核心
- `nitro-imports.d.ts` → 全局 API（defineNitroPlugin 等）
...
### 类型依赖链
```
.nuxt/types/nitro-nuxt.d.ts (Nuxt 扩展定义)
    ↓ 引用
.nuxt/types/nitro.d.ts (Nitro 核心)
    ↓ 引用
.nuxt/types/nitro-imports.d.ts (全局 API，包含 defineNitroPlugin)
```

当覆盖 `include` 时，必须显式包含这个依赖链中的所有文件，否则 TypeScript 无法找到全局类型定义。
